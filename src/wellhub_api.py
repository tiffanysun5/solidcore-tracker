...
CANCEL_MUTATION = """
mutation cancelBookedAttendance($uniqueAttendanceIdentifier: String!) {
  cancelBookedAttendance(uniqueAttendanceIdentifier: $uniqueAttendanceIdentifier) {
    variant
    title {
      ... on Text {
        value {
          ... on Translation { key namespace params __typename }
          ... on Label { value __typename }
          __typename
        }
        __typename
      }
...
def cancel_class(attendance_id: str) -> bool:
    """Cancel a booked class using the cancelBookedAttendance mutation."""
    log.info("Cancelling booking %s", attendance_id)
    try:
        results = _gql([{
            "operationName": "cancelBookedAttendance",
            "variables": {"uniqueAttendanceIdentifier": attendance_id},
            "query": CANCEL_MUTATION,
        }])
        data    = results[0].get("data", {}).get("cancelBookedAttendance", {})
        errors  = results[0].get("errors")
        if errors:
            log.error("Cancel errors: %s", errors)
            return False
        variant = data.get("variant", "")
        log.info("Cancel response variant: %s", variant)
        # variant "success" or similar means it worked; absence of errors = success
        return True
    except Exception as exc:
        log.error("Cancellation error for %s: %s", attendance_id, exc)
        return False
